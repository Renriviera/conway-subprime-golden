...
\hypersetup{colorlinks=true,linkcolor=blue!45!black,citecolor=blue!45!black,
  urlcolor=blue!45!black,pdftitle={The golden-ratio growth of Conway's subprime closure},
  pdfauthor={OpenAI Astra; Romain Popescu},
  pdfsubject={A proof draft for the cardinality-ratio conjecture of Caragiu, Vicol, and Zaki}}
...
\title{The golden-ratio growth of\\Conway's subprime closure}
\author{OpenAI Astra \and Romain Popescu}
\date{8 September 2026\\[3pt]\small Research draft}
...
For a positive integer $m$, define Conway's subprime function by
\[
 s(m)=
 \begin{cases}
  1,&m=1,\\
 ...
 \end{cases}
\]
where $\lpf(m)$ is the least prime factor of $m$. Starting with $1$, retain
all previously generated numbers and apply the operation $(a,b)\mapsto s(a+b)$
to every pair available at the preceding stage:
\begin{equation}\label{eq:closure}
 C_0=\{1\},\qquad
 C_{n+1}=C_n\cup\{s(a+b):a,b\in C_n\}.
\end{equation}
The inputs $a$ and $b$ need not be distinct. Throughout, $\N=\{1,2,\ldots\}$.

This generation problem is studied by Caragiu, Vicol, and Zaki in
\emph{On Conway's subprime function, a covering of $\N$ and an unexpected
appearance of the golden ratio} \cite{CVZ}. Their Theorem~1 proves
\begin{equation}\label{eq:exhaustion}
 \bigcup_{n\ge0}C_n=\N,
\end{equation}
and their Conjecture~3 asks whether
\begin{equation}\label{eq:conjecture}
 \lim_{n\to\infty}\frac{|C_{n+1}|}{|C_n|}
 =\ph,\qquad \ph=\frac{1+\sqrt5}{2}.
\end{equation}
...
The argument below proves the theorem using the cited exhaustion result,
...
Appendix~\ref{app:binary}. This manuscript records a proof draft; it has not
...
All logarithms are natural. The notations $f=O_A(g)$ and $f\ll_A g$
mean $|f|\le C_Ag$ for all sufficiently large values of the relevant
parameter. Any further fixed dependencies will be stated. We write
$f\asymp g$ when both $f\ll g$ and $g\ll f$ hold, and $f\sim g$
when $f/g\to1$. Let $\Pp$ denote the set of primes and $\pi(X)$
the number of primes at most $X$. Intervals used to count missing
elements are intersected with $\N$. Define
\[
...
\begin{proof}
The $q=1$, zero-frequency case of \cite[Proposition~24]{Tao} gives,
for every fixed $D>0$,
\[
...
\begin{proof}
The corollary to Theorem~1 of Coppola--Laporta
\cite[p.~246]{CL}, with exponent $5/8+1/8=3/4$, states that all but
...
The proof is given in Appendix~\ref{app:binary}. It uses no assertion
...
\section{Cardinalities and limiting density}
...
The major-arc estimate we use is \cite[Proposition~24]{Tao}:
